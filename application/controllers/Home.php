...
class Home extends MY_Controller
{
    ...
    public function index($page = 0)
    {
        $data = array();
        $head = array();
        $arrSeo = $this->CI->Public_model->getSeo('home');
        $head['title'] = @$arrSeo['title'];
        $head['description'] = @$arrSeo['description'];
        $head['keywords'] = str_replace(" ", ",", $head['title']);
        $all_categories = $this->Public_model->getShopCategores();
        ...
        $data['products'] = $this->Public_model->getProduct($this->num_rows, $page, $_GET);
